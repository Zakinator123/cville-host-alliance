...
export const actionAlerts: ActionAlert[] = [
  {
    ...
    emailTemplate: [
      ...
      '',
      'Dear City Council,',
      'My name is [YOUR NAME] and I live in [NEIGHBORHOOD]. I\'m writing on behalf of The Cville STR Advocacy Group to ask you to **PAUSE** further action on STR regulations until there is a transparent, inclusive process that solicits input on both the current regulations and proposed new regulations, and involves all stakeholders.',
      ...
      '• **What problems are we actually solving?** Why do STRs need regulation beyond existing noise and parking ordinances? The documented fire incidents in Charlottesville have occurred in traditional commercial residential rentals, not resident-operated STRs or homestays.',
      '• **Why are current STR regulations not being applied equitably across all zones?** Out-of-town and local developers operating at scale are not held to the same safety or training standards, despite contributing more significantly to impacts. The playing field between resident-operators and developers in the same neighborhoods is not level.',
      '• **How do STRs actually correlate with affordable housing?** (NYC banned STRs—rents still rose.) We should explore allocating taxes collected by STRs for Affordable Housing once "Affordable" has been defined.',
      '• **How can STR regulations incentivize new housing creation?** How can STR regulations help offset the costs needed to add new units of missing-middle housing?',
      '• **Who has been meaningfully consulted?** We need to ensure that NDS, the Planning Commission, and City Council members have adequately heard and considered perspectives from all stakeholders, not solely from individuals who submit complaints to NDS—particularly where those complaints are not formally documented or where multiple complaints may originate from the same individuals.',
      'We fully support improving safety and education, but if these measures are truly imperative, perhaps there is an opportunity to address those larger operators first, while engaging residents in an open dialogue about how to modernize the STR and homestay codes.',
      'A pause will give us time to clarify what the City is trying to achieve, present ideas from all stakeholders, and craft a framework that encourages responsible management while exploring how STRs can help the City—by enabling residents and young families to afford homeownership, supporting local tourism, and creating supplemental income and jobs for local households.',
...
export function getActiveActions(): ActionAlert[] {
  const now = new Date().toISOString();
  return actionAlerts.filter(
    (action) => !action.deadline || action.deadline >= now
  ).sort((a, b) => {
